...
const GallerySection = ( { images }: Props) => {
    ...
    return (
        <div className="bg-white px-[20px] py-[40px] md:px-[30px] md:py-[50px] lg:px-[0px] lg:py-[60px]">
            <div className={`bg-white p-[10px] gap-[23px] grid grid-cols-1 xsm:grid-cols-2 md:grid-cols-5 max-w-[1100px] m-auto`}>
                {images.map((image: any) => (
                    <figure
                        key={image.id}
                        className={`border-8 border-gray-200 border-solid m-0 aspect-w-1 aspect-h-1`}
